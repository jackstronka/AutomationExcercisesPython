...
@pytest.mark.parametrize("user", invoice_user_data())
def test_download_invoice_after_purchase(driver, base_url, user):
    ...
    assert cart_page.is_cart_page_visible(), "Cart page should be visible"
    ...
    order_success_page.click_download_invoice()
    assert order_success_page.wait_until_continue_clickable(), (
        "Continue button on payment_done page should be clickable after Download Invoice click"
    )
    order_success_page.click_continue()
